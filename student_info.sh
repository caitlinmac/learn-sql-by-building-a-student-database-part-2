...
echo "$($PSQL "SELECT course FROM courses LEFT JOIN majors_courses USING(course_id) LEFT JOIN students USING(major_id) WHERE student_id IS NOT NULL GROUP BY course HAVING COUNT(course)=1 ORDER BY course;")"
